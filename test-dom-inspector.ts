...
async function analyzeDOMStructure() {
  ...
  const domAnalysis = await page.evaluate(() => {
    ...
    const sidebarCandidates = [
      ...
      Array.from(document.querySelectorAll('*')).find(el => 
        el.textContent?.includes('Assignees') && 
    ...
    if (actualSidebar) {
      ...
      result.sidebar = {
        ...
        width: (actualSidebar as HTMLElement).offsetWidth,
        computedStyle: {
    ...
    if (actualMainContent) {
      result.mainContent = {
        ...
        width: (actualMainContent as HTMLElement).offsetWidth,
        computedStyle: {
    ...
    if (borderGridCells.length > 0) {
      result.borderGrid = {
        ...
        cells: Array.from(borderGridCells).map((cell, index) => ({
          ...
          offsetWidth: (cell as HTMLElement).offsetWidth,
  ...
  const analysisFile = path.join(OUTPUT_DIR, 'dom-analysis.json');
  fs.writeFileSync(analysisFile, JSON.stringify(domAnalysis, null, 2));
  ...
  if (domAnalysis.borderGrid) {
    ...
    domAnalysis.borderGrid.cells.forEach((cell: any) => {
