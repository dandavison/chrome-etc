...
const SCREENSHOTS_DIR = path.join(__dirname, 'width-modes-screenshots');
...
async function testWidthModes() {
  ...
  await page.screenshot({
    path: path.join(SCREENSHOTS_DIR, '1-normal-mode.png'),
    fullPage: false
  });
  ...
  await page.screenshot({
    path: path.join(SCREENSHOTS_DIR, '2-wide-mode.png'),
    fullPage: false
  });
  ...
  const wideWidth = await page.evaluate(() => {
    ...
    return content ? (content as HTMLElement).offsetWidth : 0;
  });
  ...
  await page.screenshot({
    path: path.join(SCREENSHOTS_DIR, '3-ultra-wide-mode.png'),
    fullPage: false
  });
  ...
  await page.screenshot({
    path: path.join(SCREENSHOTS_DIR, '4-normal-restored.png'),
    fullPage: false
  });
